...
std::vector<BoardPosition> Rook::getValidMoves(ChessBoard& board, const BoardPosition& pos)
{
    ...
    {
        // Check all directions (left, up, right, down)
        for (i = 1; i < 8; i++)    
        {
            BoardPosition offset = {xCord + (-1 * i), yCord};

            if (board.isInBounds(offset) &&
               (!board.isPiece(offset) || board.isBlack(offset)))
            {
                validMoves.push_back(offset);
            }
            else if (board.isBlack(offset))
            {
                validMoves.push_back(offset);
                break;
            }
...
void Rook::rookTravel(ChessBoard& board, const BoardPosition startPos, int xDir, int yDir, std::vector<BoardPosition>& moveVec)
{
    for (int i = 1; i < 8; i++)
    {
        BoardPosition offsetPos = {startPos.x() + (xDir * i), startPos.y() + yDir * i};
        if (board.isInBounds(offsetPos))
        {
            ...
            else if (board.at(offsetPos)->getColor() == board.at(startPos)->getColor())
            {
                break;
            }
            else if (board.at(offsetPos)->getColor() != board.at(startPos)->getColor())
